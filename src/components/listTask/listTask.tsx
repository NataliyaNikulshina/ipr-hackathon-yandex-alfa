    ...
    <div className={styleTask.container}>
      <ul className={styleTask.list}>
        {tasks?.length ? (
              ...
              <Task
                ...
                onClick={
                  isBoss
                    ? () =>
                        navigate(
                          `/employee-ipr/${param.id}/list-tasks/${param.idIpr}/task/${el.id}`,
                          { state: location.state }
                        )
                    : () =>
                        navigate(
                          `/myiprs/myipr/${param!.idMyIpr}/my-task/${el.id}`,
                          {
                            state: [
                              {
                                path: `/myiprs/myipr/${param!.idMyIpr}`,
                                url: url,
                                title: "Мои ИПР",
                              },
                            ],
          ...
          <li className={styleTask.title__empty}>
            {" "}
            {isSelectedIprId < 0 ? "Задач не существует" : "Выберите ИПР"}
          </li>
        )}
      </ul>
    </div>
